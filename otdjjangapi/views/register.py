...
def register_user(request):
    ...
    for field in required_fields:
        if field not in req_body:
            ...
            return HttpResponse(json.dumps(error_message), content_type='application/json', status=status.HTTP_400_BAD_REQUEST)

    # Check if the username or email already exists
    if User.objects.filter(username=req_body['username']).exists():
        return HttpResponse(json.dumps({'error': 'Username already exists'}), content_type='application/json', status=status.HTTP_409_CONFLICT)
    if User.objects.filter(email=req_body['email']).exists():
        return HttpResponse(json.dumps({'error': 'Email already exists'}), content_type='application/json', status=status.HTTP_409_CONFLICT)
